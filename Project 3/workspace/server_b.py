...
def handle_create_event(event):
    ...
    if not os.path.exists(destination + ".lock"):
...
def handle_delete_event(event):
    ...
    try:
        if os.path.exists(remote_file + ".lock"):
            files_to_delete.add(remote_file)
        elif not os.path.exists(remote_file + ".lock"):
            rmf(remote_file)
...
def handle_modify_event(event):
    ...
    if os.path.isdir(event.src_path) or fname[0] == ".":
        return None
    destination = os.path.join(SERVER_A_DIR, fname)
    if not os.path.exists(destination + ".lock") and os.path.exists(destination):
